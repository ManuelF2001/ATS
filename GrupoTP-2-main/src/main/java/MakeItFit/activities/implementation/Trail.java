...
public class Trail extends DistanceWithAltimetry implements HardInterface, Serializable {
    ...
    }*/ //PODE DAR ERRO SE FOR MAIS A DESCER Q SOBIR COMO SE GANHASSE CALORIAS A FAZER EXERCICIO

    public int caloricWaste(float index) {
        double gasto = (getDistance() * 0.5 + getElevationGain() * 0.1 - getElevationLoss() * 0.1) * index * 0.01;
        return Math.max(0, (int) gasto);
    }
