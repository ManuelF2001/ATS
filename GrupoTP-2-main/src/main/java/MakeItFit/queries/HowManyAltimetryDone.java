...
/** esta n estava a testar se estava dentro das datas
 *     public double executeQuery(UserManager userMaager, String email, MakeItFitDate date1, MakeItFitDate date2) {
 *
 *         if ((date1 != null && date2 != null) && date2.isAfter(date1)) {
 *
 *             User user = userManager.getUserByEmail(email);
 ...
 *             for (Activity a : user.getListActivities()) {
 *
 *                 if (a instanceof DistanceWithAltimetry)  { //AQUI ESTAVA O PROBLEMA
 *                     altimetry += ((DistanceWithAltimetry) a).getElevationGain()+((DistanceWithAltimetry) a).getElevationLoss();
 *                 }
 *             }
 *             return altimetry;
 ...
 *
 *
 *    //este esta a sumar em vez de subtraiar a altimetry
 *     public double executeQuery(UserManager userManager, String email) {
 *
 *         User user = userManager.getUserByEmail(email);
 *         double altimetry = 0;
 *         for (Activity a : user.getListActivities()) {
 *             if (a instanceof DistanceWithAltimetry) {
 *                 altimetry += ((DistanceWithAltimetry) a).getElevationGain() - ((DistanceWithAltimetry) a).getElevationLoss(); //DEVIA SOMAR
 *             }
 *         }
 *         return altimetry;
 *     }
